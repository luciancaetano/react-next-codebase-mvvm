...
function createComponent(baseDir, rootDirOrPath, location, name) {
    ...
    const basePath = location === 'custom' ? path.resolve(`${rootDirOrPath}/${pascalName}`) : path.resolve(`${rootDirOrPath}/${isComponent ? 'components/' : ''}${locationTypeMap[location]}/${folderName}`);
    ...
    fs.writeFileSync(`${basePath}/styles/${isPage ? kebabName : pascalName}.module.scss`, ejs.render(String(Templates.scss), params, {beautify: false}));
    console.log(`Created file: styles/${isPage ? kebabName : pascalName}.module.scss`);

    fs.writeFileSync(`${basePath}/${isPage ? kebabName : pascalName}.types.ts`, ejs.render(String(Templates.types), params));
    console.log(`Created file: ${isPage ? kebabName : pascalName}.types.ts`);

    if (!isPage) {
        fs.writeFileSync(`${basePath}/${pascalName}.tsx`, ejs.render(String(Templates.component), params));
    ...
    fs.writeFileSync(`${basePath}/${isPage ? kebabName : pascalName}.view-model.ts`, ejs.render(String(Templates.viewModel), params));
